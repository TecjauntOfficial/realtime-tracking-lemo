...
io.on("connection", (socket) => {
  ...
  socket.on("userJoined", (data) => {
    ...
    socket.emit(
      "userList",
      Array.from(io.sockets.sockets.values()).map((s) => s.username)
    );
    io.emit(
      "userList",
      Array.from(io.sockets.sockets.values()).map((s) => s.username)
  ...
  socket.on("messageReceived", (data) => {
    console.log(
      `[${data.timestamp}] Message from ${data.sender}:`,
      data.message
    );
  ...
  socket.on("joinDashboard", () => {
    ...
    console.log(
      "Sockets in dashboard:",
      io.sockets.adapter.rooms.get("dashboard")?.size || 0
    );
...
chatNamespace.on("connection", (socket) => {
  ...
  socket.on("userJoined", (data) => {
    ...
    chatNamespace.emit(
      "userList",
      Array.from(chatNamespace.sockets.values()).map((s) => s.username)
    );
    console.log(`${name} has joined the chat.`);
  });
